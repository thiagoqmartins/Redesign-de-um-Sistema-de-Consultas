...
# from scripts.createZZ import buscar_dados
...
def executar(session):  
    ...
    for i in range(nRow):
        ...
        dataCorte = datetime.strptime("01.08.2019","%d.%m.%Y")
...
def sequencia_execucao():
    with sqlite3.connect('BD/banco_dados.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT COALESCE(MAX(seq_exec), 0) FROM consultas_iqs9")
